...
import com.finalproject.service.TwoFactorAuth;
...
//    }
  ...
  public ResponseEntity<?> registerUser(@Valid @RequestBody SignupRequest signUpRequest) {
    ...
    if (signUpRequest.isMfa()) {
      ...
      signUpRequest.setSecret(twoFactorAuth.generateSecret());
    }
    ...
    if (strRoles == null) {
      ...
      strRoles.forEach(role -> {
        ...
        case "admin":
          ...
          break;
          case "student":
            Role studentRole = roleRepository.findByName(RoleEnum.ROLE_STUDENT).orElseThrow(() -> new RuntimeException("Error: Role is not found"));
            roles.add(studentRole);
          break;
          case "faculty":
            Role facultyRole = roleRepository.findByName(RoleEnum.ROLE_FACULTY).orElseThrow(() -> new RuntimeException("Error: Role is not found"));
    ...
    // Create new student
    if (roles.stream().toList().get(0).getName().equals(RoleEnum.ROLE_STUDENT)) {
...
//    return ResponseEntity.ok(new MessageResponse("User registered successfully!"));
    ...
    return ResponseEntity.created(location)
            .body(new SignupResponse(user1.isMfa(), twoFactorAuth.getUriForImage(user.getSecret())));
  }
}
